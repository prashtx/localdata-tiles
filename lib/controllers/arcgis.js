...
var arcGISSource = require('../datasources/arcgis');
...
function getMap(options, callback) {
  ...
  // TODO: Process the styles to determine which fields we need from the
  // datasource. In the meantime, we get all fields.
  var datasource = arcGISSource.create({
    query: query
  });
...
exports.renderGrids = function renderGrids(req, res, next) {
  ...
  layer.getDefinition({
    ...
    var defaultSelect = {};
    defaultSelect[layerDef.query.pkey] = 1;
    options.select = _.defaults(defaultSelect, layerDef.select);
